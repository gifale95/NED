...
def synthesize_fmri_responses(ned_dir, images, train_dataset, subject, roi,
	model, device):
	"""
	Synthesize fMRI responses for the input images using the chosen trained
	encoding model.
	...
	### Synthesize fMRI responses to images ###
	if train_dataset == 'nsd':

		if model == 'fwrf':

			# Synthesize fMRI responses to images using the feature-weighted
			# receptive field (fwrf) encoding model (St-Yves & Naselaris, 2018).
			synthetic_fmri_responses = fmri_nsd_fwrf(
				ned_dir,
				images,
				subject,
				roi,
...
def synthesize_eeg_responses(ned_dir, images, train_dataset, subject, model,
	device):
	"""
	Synthesize EEG responses for the input images using the chosen trained
	encoding model.
	...
	roi : str
		Name of the Region of Interest (ROI) for which the EEG image
		responses are synthesized.
	model : str
	...
	if train_dataset == 'things_eeg_2':
		...
		if model == 'vit_b_32':
			...
			# Synthesize EEG responses to images using a linear mapping of
			# a pre-trained vision transformer (Dosovitskiy et al., 2020) image
			# features.
			synthetic_eeg_responses = eeg_things_eeg_2_vit_b_32(
				ned_dir,
				images,
				subject,
				device
...
def get_fmri_metadata(ned_dir, train_dataset, subject, roi, model):
	"""
	Get the synthetic fMRI metadata.
...
def get_eeg_metadata(ned_dir, train_dataset, subject, model):
	"""
	Get the synthetic EEG metadata.
